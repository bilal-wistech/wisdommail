...
@section('content')
    ...
    <form class="" role="form" method="POST" action="{{ url('/login') }}">
        ...
        <div class="panel panel-body p-4 rounded-3 bg-white shadow">
            ...
            <div
                class="form-group login-email-input has-feedback has-feedback-left{{ $errors->has('email') ? ' has-error' : '' }}">
            ...
            <div
                class="form-group login-password-input has-feedback has-feedback-left{{ $errors->has('password') ? ' has-error' : '' }}">
                <input id="password" type="password" class="form-control" name="password"
                    placeholder="{{ trans('messages.password') }}" value="{{ demo_auth()['password'] }}">
                <div class="form-control-feedback">
            ...
            <div class="form-group login-options mt-4">
                <div class="row align-items-center small">
                    ...
                    <div class="col-sm-6 text-end text-semibold fw-600">
                        <font size="-3"><a
                                href="{{ url('/password/reset') }}">{{ trans('messages.forgot_password') }}</a></font>
                    </div>
                </div>
            </div>
            ...
            <button type="submit" class="btn rounded-2 btn-primary d-block login-button py-2 fw-600"
                style="width:100%;text-transform:uppercase"><span class="me-2">{{ trans('messages.login') }}</span> <span
                    ...
                    @if (\Acelle\Model\Setting::get('oauth.google_enabled') == 'yes')
                        <div class="text-center mt-2">
                            <a href="{{ action('AuthController@googleRedirect', [
                                ...
                                <span class="ms-3 display-6 text-center"
                                    style="font-size:15px;width:80%">{{ trans('messages.continue_with_google') }}</span>
                            </a>
                        </div>
                    @endif
                    ...
                    @if (\Acelle\Model\Setting::get('oauth.facebook_enabled') == 'yes')
                        <div class="text-center mt-2">
                            <a href="{{ action('AuthController@facebookRedirect') }}"
                                ...
                                <span class="ms-3 display-6 text-center"
                                    style="font-size:15px;width:80%">{{ trans('messages.continue_with_facebook') }}</span>
                            </a>
                        </div>
                    @endif
                </div>

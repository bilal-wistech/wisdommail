        ...
        @foreach ($subscribers as $key => $item)
            <tr>
                <td width="1%">
                    <div class="text-nowrap">
                        <div class="checkbox inline me-1">
                            <label>
                                <input type="checkbox" class="node styled" name="uids[]" value="{{ $item->uid }}" />
                            </label>
                        </div>
                    </div>
                </td>
                    ...
                    <div class="d-flex align-items-center">
                        <div class="subscriber-avatar mt-1">
                            <a image-popup="link"
                                ...
                                <img
                                    src="{{ isSiteDemo() ? 'https://i.pravatar.cc/300?v=' . $key : action('SubscriberController@avatar', $item->uid) }}" />
                            </a>
                        </div>
                        <div class="no-margin text-bold">
                            <a class="kq_search"
                                href="{{ action('SubscriberController@edit', ['list_uid' => $list->uid, 'uid' => $item->uid]) }}">
                                {{ $item->email }}
                            </a>
                            <br />
                            <span
                                class="label label-flat bg-{{ $item->status }}">{{ trans('messages.' . $item->status) }}</span>
                            <span
                                class="label label-flat bg-{{ $item->verification_status }}">{{ trans('messages.email_verification_result_' . $item->verification_status) }}</span>
                        </div>
                    </div>
                </td>
                ...
                @if (in_array('created_at', is_array(request()->columns) ? request()->columns : []))
                    <td>
                        <span
                            class="no-margin stat-num">{{ Auth::user()->customer->formatDateTime($item->created_at, 'datetime_full') }}</span>
                        <br>
                ...
                @if (in_array('updated_at', is_array(request()->columns) ? request()->columns : []))
                    <td>
                        <span
                            class="no-margin stat-num">{{ Auth::user()->customer->formatDateTime($item->updated_at, 'datetime_full') }}</span>
                        <br>
                    ...
                    <div class="btn-group">
                        ...
                        <ul class="dropdown-menu dropdown-menu-end">
                            @if (\Gate::allows('subscribe', $item))
                                <li><a class="dropdown-item list-action-single" link-method="POST"
                                        href="{{ action('SubscriberController@subscribe', ['list_uid' => $list->uid, 'uids' => $item->uid]) }}"><span
                                            class="material-symbols-rounded">mark_email_read</span>
                                        {{ trans('messages.subscribe') }}</a></li>
                            @endif
                            @if (\Gate::allows('unsubscribe', $item))
                                <li><a class="dropdown-item list-action-single" link-method="POST"
                                        href="{{ action('SubscriberController@unsubscribe', ['list_uid' => $list->uid, 'uids' => $item->uid]) }}"><span
                                            class="material-symbols-rounded">logout</span>
                                        {{ trans('messages.unsubscribe') }}</a></li>
                            @endif
                            ...
                            @if (\Gate::allows('update', $item))
                                <li>
                                    <a class="dropdown-item list-action-single" link-method="POST"
                                        ...
                                        href="{{ action('SubscriberController@resendConfirmationEmail', ['list_uid' => $list->uid, 'uids' => $item->uid]) }}">
                                        <span class="material-symbols-rounded">mark_email_read</span>
                                        {{ trans('messages.subscribers.resend_confirmation_email') }}
                                    </a>
                                </li>
                            @endif
                            @if (\Gate::allows('delete', $item))
                                <li><a class="dropdown-item list-action-single"
                                        ...
                                        href="{{ action('SubscriberController@delete', ['list_uid' => $list->uid, 'uids' => $item->uid]) }}"><span
                                            class="material-symbols-rounded">delete_outline</span>

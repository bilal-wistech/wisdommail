...
class Kernel extends ConsoleKernel
{
    ...
    protected function schedule(Schedule $schedule)
    {
        ...
        // sync lists
        $schedule->command('sync:maillists')->dailyAt('00:00'); // run at 00:00 am everyday
        ...
        */
        $schedule->call(function () {
            Log::info('Starting the update_list_stats task.');

            Customer::with(['lists'])
                ->chunk(100, function ($customers) {
                    foreach ($customers as $customer) {
                        // Log customer processing
                        Log::info("Processing customer ID: {$customer->id}");

                        // Skip customers without an active general subscription
                        if (is_null($customer->getCurrentActiveGeneralSubscription())) {
                            Log::info("Skipping customer ID: {$customer->id}, no active general subscription.");
                            continue;
                        }

                        // Process customer's lists
                        foreach ($customer->lists as $list) {
                            Log::info("Dispatching UpdateMailListJob for list ID: {$list->id}");
                            safe_dispatch(new \Acelle\Jobs\UpdateMailListJob($list));
                        }

                        // Dispatch job for the customer
                        Log::info("Dispatching UpdateUserJob for customer ID: {$customer->id}");
                        safe_dispatch(new \Acelle\Jobs\UpdateUserJob($customer));
                    }
                });

            Log::info('Completed the update_list_stats task.');
        })->name('update_list_stats')->daily();
